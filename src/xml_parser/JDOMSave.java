...
public class JDOMSave {
	...
	static org.jdom2.Element racine = new org.jdom2.Element("SmallWorld");
	static org.jdom2.Document document = new Document(racine);
	
	 public void SavetoXML(SmallWorld sw){
					 ...
					 for( int k = 0; k < map[i][j].getElementsList().size(); k++){
						 ...
						 if(elem instanceof Resource){
							 ...
							 if(elem instanceof Food){//The "name" attribute has to be set 
								 Attribute name = new Attribute("name", "food");
								 re.setAttribute(name);
							 }
							 else if(elem instanceof Metal){
								 Attribute name = new Attribute("name", "metal");
								 re.setAttribute(name);
							 }
							 else if(elem instanceof Plutonium){
								 Attribute name = new Attribute("name", "plutonium");
								 re.setAttribute(name);
							 }
							 else if(elem instanceof Rock){
								 Attribute name = new Attribute("name", "rock");
								 re.setAttribute(name);
							 }
							 else if(elem instanceof Wood){
								 Attribute name = new Attribute("name", "wood");
								 re.setAttribute(name);
							 }
